...
<table class="table table-bordered">
    ...
    <c:forEach var="article" items="${articles}">
        ...
        <!-- 👇 Zone commentaire pour les clients -->
        <c:if test="${sessionScope.role == 'client'}">
        <tr>
            <td colspan="5">
                <form method="post" action="CommentaireServlet" class="d-flex align-items-center">
                    <input type="hidden" name="article_id" value="${article.id}" />
                    <input type="text" name="contenu" class="form-control me-2" placeholder="Écrire un commentaire..." required />
                    <button type="submit" class="btn btn-outline-success btn-sm">Commenter</button>
                </form>
                <ul class="mt-2">
